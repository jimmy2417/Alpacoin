...
// Copyright (c) 2013 AlpaCoin developers
// Copyright (c) 2014 AlpaCoin developers
...
const string strMessageMagic = "AlpaCoin Signed Message:\n";

double dHashesPerSec = 0.0;
int64 nHPSTimerStart = 0;

// Settings
int64 nTransactionFee = 0.0001;
...
/** Amount of AlpaCoins spent by the transaction.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight > 5400000)	// Stop reward
		nSubsidy = 0;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AlpaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AlpaCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running AlpaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AlpaCoinMiner terminated\n");
        throw;
    }
}
